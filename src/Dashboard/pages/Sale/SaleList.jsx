...
export default function SaleList() {
  const perPage = 10;
  const API = `http://192.168.1.157:8082/masterservice/api/sale`;
  ...
  const handleDelete = async () => {
    try {
      ...
      toast.success(
        courseStatusToDelete ? "SaleList Deactivated." : "SaleList Activated."
      );
    } catch (error) {
  ...
  const applyFilters = () => {
    let filteredList = SaleListList.filter((sale) => {
      ...
      if (
        billNumberFilter &&
        !sale.billNumber.toLowerCase().includes(billNumberFilter.toLowerCase())
      ) {
        return false;
      }
