...
const Sales = () => {
  ...
  const handleAddProduct = () => {
    ...
    const existingProduct = saleDetailsList.find(
      (product) =>
  ...
  const fetchCategoryList = async () => {
    try {
      const response = await axios.get(
        "http://192.168.1.157:8082/masterservice/api/category/get-categories-list"
      );
  ...
  const fetchProductList = async (categoryId) => {
    try {
      const response = await axios.get(
        `http://192.168.1.157:8082/masterservice/api/products/get-delivered-products/${categoryId}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API}/add`,
        { saleDetailsList, customerName, contactNo, date },
        {
          headers: {
  ...
  const handleDelete = async (e) => {
    try {
      ...
      const existingProductIndex = saleDetailsList.findIndex(
        (product) =>
