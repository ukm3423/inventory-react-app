...
function PurchaseReport() {
    ...
    const filteredOrderList = purchaseData.filter((purchase) => {
        return (
            (!orderNumberFilter ||
                (purchase.invoiceNo && purchase.invoiceNo.toLowerCase().includes(orderNumberFilter.toLowerCase())) ||
                (purchase.orderNo && purchase.orderNo.toLowerCase().includes(orderNumberFilter.toLowerCase()))) &&
            (!fromDateFilter || new Date(purchase.orderDate) >= new Date(fromDateFilter)) &&
    ...
    if (loading) {
        return <div className="container mx-auto p-6 bg-white rounded-lg shadow-md">Loading...</div>;
    }

    if (error) {
        return <div className="container mx-auto p-6 bg-white rounded-lg shadow-md">{error}</div>;
    }
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {filteredOrderList.map((purchase, index) => (
                                    <tr key={index} className="transition duration-300 ease-in-out hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-2 text-gray-800 whitespace-nowrap">{purchase.orderNo}</td>
                                        <td className="px-6 py-2 text-gray-800 whitespace-nowrap">{purchase.orderDate}</td>
                                        <td className="px-6 py-2 text-gray-800 whitespace-nowrap">{purchase.invoiceNo}</td>
                                        <td className="px-6 py-2 text-gray-800 whitespace-nowrap">{purchase.orderDate}</td>

                                        <td className="px-6 py-2 text-gray-800 whitespace-nowrap">{purchase.supplier}</td>
                                        <td className="px-6 py-2 text-gray-800 whitespace-nowrap">{purchase.amount}</td>
