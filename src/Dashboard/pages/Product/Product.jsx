...
const Product = () => {
  ...
  const perPage = 100;
  const API = `http://192.168.1.157:8082/masterservice/api/products`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${API}/add`,
        { productName, productCode, price, categoryId },
        {
          headers: {
  ...
  const handleDelete = async () => {
    try {
      ...
      toast.success(
        productStatusToDelete ? "Category Deactivated." : "Category Activated."
      );
    } catch (error) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-1 gap-6">
        <div className="container mx-auto py-8">
          ...
          <form onSubmit={handleSubmit} className="mb-8">
            ...
            <button
              type="submit"
              className="px-3 py-2 bg-blue-500 text-white rounded-md hover:bg-blue-600 mt-4"
            >
              Add Product
            </button>
          </form>
