...
const UpdateModal = ({ order, onClose, onUpdate }) => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await axios.post(
        `http://192.168.1.157:8082/masterservice/api/delivery/make-delivery`,
        {
          orderCode: orderCode,
  ...
  return (
    ...
    >
      <div
        className={`bg-white w-full max-w-6xl rounded-lg shadow-lg overflow-hidden transform transition-transform duration-300  ${
          modalOpen ? "scale-100" : "scale-90"
        }`}
      >
        <div className="bg-gradient-to-r from-pink-400 to-red-500 px-4 py-3 sm:px-6">
          <h3 className="text-lg leading-8 font-medium text-white">
            Deliver Order
          </h3>
        </div>
